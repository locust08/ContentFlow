...
export function ApprovalsPage({ app }) {
  ...
  async function reviewScript(project, status) { await app.reviewProjectScript(project.name, status, feedback[project.name] || ""); }
  return <div className="page-stack"><PageHeading eyebrow="Review" title="Approvals" copy="Review scripts and final campaign outputs with a complete audit trail." /><div className="approval-grid">{app.reviewItems.length ? app.reviewItems.map((project) => {
    const scriptReview = project.scriptStatus === "client-review";
    const status = scriptReview ? project.scriptStatus : project.approvalStatus;
    return <Card key={project.name} eyebrow={`${byId(app.organization.clients, project.clientId, "No client")} · ${scriptReview ? "UGC script" : "Final media"}`} title={project.name} action={<Badge tone={status}>{status}</Badge>}><textarea aria-label={`Feedback for ${project.name}`} value={feedback[project.name] ?? project.approvalFeedback ?? ""} onChange={(event) => setFeedback((current) => ({ ...current, [project.name]: event.target.value }))} placeholder="Review feedback" /><div className="inline-actions"><Button variant="secondary" onClick={() => scriptReview ? reviewScript(project, "changes-requested") : review(project, "changes-requested")}>Request changes</Button><Button onClick={() => scriptReview ? reviewScript(project, "approved") : review(project, "approved")}><Check size={16} /> Approve</Button></div></Card>;
  }) : <EmptyState>No project has been submitted for review yet.</EmptyState>}</div></div>;
}
...
export function AnalyticsPage({ app }) {
  ...
  return <div className="page-stack"><PageHeading eyebrow="Reporting" title="Analytics" copy="Production, research, scripting, and approval performance from Supabase records." />
    <section className="metric-grid">
      <MetricCard label="Total projects" value={analytics.projects ?? app.projects.length} caption="Managed workspaces" />
      <MetricCard label="Market reports" value={analytics.market_reports || 0} caption="Research reports created" />
      <MetricCard label="UGC scripts" value={analytics.ugc_scripts || 0} caption="Scripts generated" />
      <MetricCard label="Revisions" value={analytics.script_revisions || 0} caption="Versioned script edits" />
      <MetricCard label="Approval time" value={`${analytics.script_approval_average_hours || 0}h`} caption="Average script review" />
      <MetricCard label="Script to render" value={`${analytics.script_to_render_conversion || 0}%`} caption="Production conversion" />
      <MetricCard label="Clip candidates" value={analytics.clip_candidates || 0} caption="Detected moments" />
      <MetricCard label="Approved" value={approval.approved || 0} caption="Client-approved projects" />
    </section>
    <section className="analytics-dashboard">
      <Card eyebrow="Production mix" title="Projects by workflow"><div className="horizontal-chart"><span><i style={{ width: `${Math.max(8, Number(typeStats["ai-generator"] || analytics.ai_projects || 0) * 12)}%` }} />AI Generator <strong>{typeStats["ai-generator"] || analytics.ai_projects || 0}</strong></span><span><i className="purple" style={{ width: `${Math.max(8, Number(typeStats["auto-clipper"] || analytics.clipper_projects || 0) * 12)}%` }} />Auto Clipper <strong>{typeStats["auto-clipper"] || analytics.clipper_projects || 0}</strong></span></div></Card>
      <Card eyebrow="Approval health" title="Review status"><div className="donut-summary"><div className="donut" /><ul><li><i className="approved" /> Approved <strong>{approval.approved || 0}</strong></li><li><i className="review" /> In review <strong>{approval["in-review"] || 0}</strong></li><li><i className="changes" /> Changes <strong>{approval["changes-requested"] || 0}</strong></li></ul></div></Card>
      <Card eyebrow="Creative strategy" title="Hook and angle distribution"><div className="horizontal-chart">{(analytics.hook_angle_breakdown || []).map((item) => <span key={item.name}><i className="purple" style={{ width: `${Math.max(8, Number(item.count || 0) * 14)}%` }} />{item.name} <strong>{item.count}</strong></span>)}{!analytics.hook_angle_breakdown?.length && <small>No approved script data yet.</small>}</div></Card>
      <Card eyebrow="Production trend" title="Monthly renders"><div className="horizontal-chart">{(analytics.monthly_renders || []).map((item) => <span key={item.month}><i style={{ width: `${Math.max(8, Number(item.count || 0) * 12)}%` }} />{item.month} <strong>{item.count}</strong></span>)}{!analytics.monthly_renders?.length && <small>No render trend yet.</small>}</div></Card>
    </section>
  </div>;
}
